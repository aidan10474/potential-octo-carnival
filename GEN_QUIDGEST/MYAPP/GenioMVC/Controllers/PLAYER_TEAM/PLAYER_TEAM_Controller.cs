...
namespace GenioMVC.Controllers
{
	public partial class Player_teamController : ControllerBase
	{
		...
		public Player_teamController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL PRJ CONTROLLER_NAVIGATION PLAYER_TEAM]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAplayer_team>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL PRJ MANUAL_CONTROLLER PLAYER_TEAM]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Player_team row = new Models.Player_team(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("player_team");
			...
			try
			{
				...
				{
					case "F_PLAYER_TEAM__PLAYER__NAME":	// Field (DB)
						{
							var model = new F_player_team_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_player_team__player__name(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_PLAYER_TEAM__PLAYER__NAME":	// Field (DB)
						values = new F_player_team_ViewModel(UserContext.Current).GetDependant_F_player_teamTablePlayerName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_player_team" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_player_team([FromBody]F_player_team_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "player_team",
				(primaryKey) => Models.Player_team.Find(primaryKey, UserContext.Current, "FF_PLAYER_TEAM"),
				(model) => formData.MapToModel(model as Models.Player_team)
			);
		}
		...
		{
			return base.GetDocumsTickets("PLAYER_TEAM", requestModel.FieldName, requestModel.KeyValue);
		}

...
namespace GenioMVC.Controllers
{
	public partial class Skill_playerController : ControllerBase
	{
		...
		public Skill_playerController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL PRJ CONTROLLER_NAVIGATION SKILL_PLAYER]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAskill_player>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL PRJ MANUAL_CONTROLLER SKILL_PLAYER]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Skill_player row = new Models.Skill_player(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("skill_player");
			...
			try
			{
				...
				{
					case "F_SKILL_PLAYER__PLAYER__NAME":	// Field (DB)
						{
							var model = new F_skill_player_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_skill_player__player__name(qs);
							result = model.TablePlayerName;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_SKILL_PLAYER__PLAYER__NAME":	// Field (DB)
						values = new F_skill_player_ViewModel(UserContext.Current).GetDependant_F_skill_playerTablePlayerName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_skill_player" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_skill_player([FromBody]F_skill_player_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "skill_player",
				(primaryKey) => Models.Skill_player.Find(primaryKey, UserContext.Current, "FF_SKILL_PLAYER"),
				(model) => formData.MapToModel(model as Models.Skill_player)
			);
		}
		...
		{
			return base.GetDocumsTickets("SKILL_PLAYER", requestModel.FieldName, requestModel.KeyValue);
		}

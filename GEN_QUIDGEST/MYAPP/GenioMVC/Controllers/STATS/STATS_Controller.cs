...
// USE /[MANUAL PRJ CONTROLLER_NAVIGATION STATS]/
...
// USE /[MANUAL PRJ MANUAL_CONTROLLER STATS]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Stats row = new Models.Stats(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("stats");
			...
			try
			{
				...
				{
					case "F_PLAYER_STATS__PLAYER__NAME":	// Field (DB)
						{
							var model = new F_player_stats_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_player_stats__player__name(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_PLAYER_STATS__PLAYER__NAME":	// Field (DB)
						values = new F_player_stats_ViewModel(UserContext.Current).GetDependant_F_player_statsTablePlayerName(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_player_stats" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_player_stats([FromBody]F_player_stats_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "stats",
				(primaryKey) => Models.Stats.Find(primaryKey, UserContext.Current, "FF_PLAYER_STATS"),
				(model) => formData.MapToModel(model as Models.Stats)
			);
		}

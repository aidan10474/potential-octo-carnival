<template>
	...
	<qtable 
		...
		<template #PRJ="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'PRJ' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					PRJ : { active: true, Cod: "PRJ", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.PRJ.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.PRJ.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.PRJ.Description),
					name: "PRJ",
					slot_name: "PRJ",
					sort: false,
					visibility: vm.Modules.PRJ.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {
